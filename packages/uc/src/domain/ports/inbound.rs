...
use crate::domain::permissions::{SecurableType, PermissionsList, UpdatePermissions};
...
pub trait VolumeUseCase: Send + Sync {
    async fn create_volume(&self, principal: &str, cmd: CreateVolume) -> Result<VolumeInfo, DomainError>;
    async fn list_volumes(&self, principal: &str, catalog_name: &str, schema_name: &str, max_results: Option<i32>, page_token: Option<String>) -> Result<ListVolumesResponse, DomainError>;
    async fn get_volume(&self, principal: &str, full_name: &str) -> Result<VolumeInfo, DomainError>;
    async fn update_volume(&self, principal: &str, full_name: &str, cmd: UpdateVolume) -> Result<VolumeInfo, DomainError>;
    async fn delete_volume(&self, principal: &str, full_name: &str) -> Result<(), DomainError>;
}

#[async_trait]
pub trait FunctionUseCase: Send + Sync {
    async fn create_function(&self, principal: &str, cmd: CreateFunction) -> Result<FunctionInfo, DomainError>;
    async fn list_functions(&self, principal: &str, catalog_name: &str, schema_name: &str, max_results: Option<i32>, page_token: Option<String>) -> Result<ListFunctionsResponse, DomainError>;
    async fn get_function(&self, principal: &str, full_name: &str) -> Result<FunctionInfo, DomainError>;
...
pub trait PermissionUseCase: Send + Sync {
    async fn get_permissions(&self, principal: &str, securable_type: SecurableType, full_name: &str) -> Result<PermissionsList, DomainError>;
    async fn update_permissions(&self, principal: &str, securable_type: SecurableType, full_name: &str, changes: UpdatePermissions) -> Result<PermissionsList, DomainError>;
}
