use crate::domain::{
    ...
    ports::{
        ...
        outbound::{AuthorizerPort, ModelVersionRepository, RegisteredModelRepository},
    },
};
...
impl ModelUseCase for ModelService {
        ...
        let schema_ok = if !schema_owner {
            self.authorizer
                .is_authorized_all(
                    ...
                    SecurableType::Schema,
                    &schema_full,
                    &[Privilege::UseSchema, Privilege::CreateModel],
                )
                .await?
        } else {
            true
    ...
    async fn delete_registered_model(
        ...
        self.authorizer
            .remove_object(SecurableType::RegisteredModel, full_name)
            .await?;
        Ok(())
    }
    ...
    async fn list_model_versions(
        ...
        if !allowed {
            return Err(DomainError::Forbidden(format!(
                "Principal {} is not authorized to perform this action on {}",
                principal, model_full_name
            )));
        }
        self.version_repo
            .list(model_full_name, max_results, page_token)
            .await
    }
    ...
    async fn update_model_version(
        ...
        self.version_repo
            .update(model_full_name, version, cmd)
            .await
    }
