...
impl FunctionUseCase for FunctionService {
    async fn create_function(&self, principal: &str, cmd: CreateFunction) -> Result<FunctionInfo, DomainError> {
        ...
        let schema_ok = if !schema_owner {
            self.authorizer
                .is_authorized_all(principal, SecurableType::Schema, &schema_full, &[Privilege::UseSchema, Privilege::CreateFunction])
                .await?
        } else {
            true
        };
        ...
        self.authorizer.grant(principal, SecurableType::Function, &full_name, Privilege::Owner).await?;
        self.authorizer.add_hierarchy(SecurableType::Function, &full_name, SecurableType::Schema, &schema_full).await?;
        Ok(info)
    }
    ...
    async fn list_functions(&self, principal: &str, catalog_name: &str, schema_name: &str, max_results: Option<i32>, page_token: Option<String>) -> Result<ListFunctionsResponse, DomainError> {
        ...
        for function in response.functions {
            let ok = self.authorizer
                .is_authorized_any(principal, SecurableType::Function, &function.full_name, &[Privilege::Owner, Privilege::Execute])
                .await
                .unwrap_or(false);
            if ok {
    ...
    async fn get_function(&self, principal: &str, full_name: &str) -> Result<FunctionInfo, DomainError> {
        let allowed = self.authorizer
            .is_authorized_any(principal, SecurableType::Function, full_name, &[Privilege::Owner, Privilege::Execute])
            .await?;
        if !allowed {
            return Err(DomainError::Forbidden(format!(
    ...
    async fn delete_function(&self, principal: &str, full_name: &str) -> Result<(), DomainError> {
        let allowed = self.authorizer
            .is_authorized(principal, SecurableType::Function, full_name, Privilege::Owner)
            .await?;
        if !allowed {
            return Err(DomainError::Forbidden(format!(
                "Principal {} is not authorized to perform this action on {}", principal, full_name
        ...
        self.authorizer.remove_object(SecurableType::Function, full_name).await?;
        Ok(())
    }
}
