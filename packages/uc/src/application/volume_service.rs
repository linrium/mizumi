...
impl VolumeUseCase for VolumeService {
    async fn create_volume(&self, principal: &str, cmd: CreateVolume) -> Result<VolumeInfo, DomainError> {
        ...
        let schema_ok = if !schema_owner {
            self.authorizer
                .is_authorized_all(principal, SecurableType::Schema, &schema_full, &[Privilege::UseSchema, Privilege::CreateVolume])
                .await?
        } else {
            true
        };
        ...
        self.authorizer.grant(principal, SecurableType::Volume, &full_name, Privilege::Owner).await?;
        self.authorizer.add_hierarchy(SecurableType::Volume, &full_name, SecurableType::Schema, &schema_full).await?;
        Ok(info)
    }
    ...
    async fn list_volumes(&self, principal: &str, catalog_name: &str, schema_name: &str, max_results: Option<i32>, page_token: Option<String>) -> Result<ListVolumesResponse, DomainError> {
        ...
        for volume in response.volumes {
            let ok = self.authorizer
                .is_authorized_any(principal, SecurableType::Volume, &volume.full_name, &[Privilege::Owner, Privilege::ReadVolume])
                .await
                .unwrap_or(false);
            if ok {
                allowed.push(volume);
    ...
    async fn get_volume(&self, principal: &str, full_name: &str) -> Result<VolumeInfo, DomainError> {
        let allowed = self.authorizer
            .is_authorized_any(principal, SecurableType::Volume, full_name, &[Privilege::Owner, Privilege::ReadVolume])
            .await?;
        if !allowed {
            return Err(DomainError::Forbidden(format!(
    ...
    async fn update_volume(&self, principal: &str, full_name: &str, cmd: UpdateVolume) -> Result<VolumeInfo, DomainError> {
        let allowed = self.authorizer
            .is_authorized(principal, SecurableType::Volume, full_name, Privilege::Owner)
            .await?;
        if !allowed {
            return Err(DomainError::Forbidden(format!(
                "Principal {} is not authorized to perform this action on {}", principal, full_name
    ...
    async fn delete_volume(&self, principal: &str, full_name: &str) -> Result<(), DomainError> {
        ...
        self.authorizer.remove_object(SecurableType::Volume, full_name).await?;
        Ok(())
    }
}
