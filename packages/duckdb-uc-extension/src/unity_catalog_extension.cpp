...
static unique_ptr<BaseSecret> CreateUCSecretFunction(ClientContext &, CreateSecretInput &input) {
	...
	for (const auto &named_param : input.options) {
		...
		} else if (lower_name == "s3_endpoint") {
			result->secret_map["s3_endpoint"] = named_param.second.ToString();
		} else if (lower_name == "s3_url_style") {
			result->secret_map["s3_url_style"] = named_param.second.ToString();
		} else if (lower_name == "s3_use_ssl") {
			result->secret_map["s3_use_ssl"] = named_param.second.ToString();
		} else {
...
static void SetUCSecretParameters(CreateSecretFunction &function) {
	...
	function.named_parameters["s3_endpoint"] = LogicalType::VARCHAR;
	function.named_parameters["s3_url_style"] = LogicalType::VARCHAR;
	function.named_parameters["s3_use_ssl"] = LogicalType::BOOLEAN;
}
...
static unique_ptr<Catalog> UnityCatalogAttach(optional_ptr<StorageExtensionInfo> storage_info, ClientContext &context,
	...
	if (secret_entry) {
		...
		Value s3_endpoint_val = kv_secret.TryGetValue("s3_endpoint");
		credentials.s3_endpoint = s3_endpoint_val.IsNull() ? "" : s3_endpoint_val.ToString();
		Value s3_url_style_val = kv_secret.TryGetValue("s3_url_style");
		credentials.s3_url_style = s3_url_style_val.IsNull() ? "" : s3_url_style_val.ToString();
		Value s3_use_ssl_val = kv_secret.TryGetValue("s3_use_ssl");
		if (!s3_use_ssl_val.IsNull()) {
			credentials.s3_use_ssl = BooleanValue::Get(s3_use_ssl_val.DefaultCastAs(LogicalType::BOOLEAN));
			credentials.s3_use_ssl_set = true;
		}
