...
def main() -> None:
    ...
    hdbank_customers_df = (
        ...
        .select(
            ...
            F.col("seed_timestamp"),
        )
    )

    vietjetair_customers_df = (
    ...
    hdbank_customers_df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save(
        HDBANK_BRONZE_TARGET_PATH
    )
    vietjetair_customers_df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save(
        VIETJETAIR_BRONZE_TARGET_PATH
    )
    ...
    with open_dagster_pipes() as pipes:
        pipes.report_asset_materialization(
            ...
            metadata={
                "row_count": hdbank_customers_df.count(),
        ...
        pipes.report_asset_materialization(
            ...
            metadata={
                "row_count": vietjetair_customers_df.count(),
                "customer_master_rows": customer_master_df.count(),
                "customer_master_export": RUSTFS_CUSTOMER_MASTER_PATH,
