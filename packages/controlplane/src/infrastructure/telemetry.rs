...
pub fn init() -> Result<TelemetryGuard, Box<dyn std::error::Error>> {
    ...
    if otel_enabled() {
        let resource = Resource::builder()
            ...
            .with_attribute(KeyValue::new("service.namespace", "controlplane"))
            .build();
...
/// Inject the W3C trace context (`traceparent` / `tracestate` headers) from
/// the active tracing span into a `reqwest::RequestBuilder`.
///
/// Call this on every outbound HTTP request so downstream services (Unity
/// Catalog, Keycloak, Dagster, MLflow) can attach their spans to the same
/// distributed trace.
pub fn inject_trace(builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
    use tracing_opentelemetry::OpenTelemetrySpanExt;
    ...
    propagator.inject_context(&context, &mut ReqwestInjector(&mut headers));
