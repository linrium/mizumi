...
type TableDetail = { name: string; catalog_name: string; schema_name: string; columns: ColumnInfo[] }
...
export async function fetchSchema(): Promise<string> {
  const catalogsData = await ucGet<{ catalogs: CatalogInfo[] }>('/catalogs?max_results=100')
  const catalogs = catalogsData?.catalogs ?? []
  if (catalogs.length === 0) return '(schema unavailable)'

  const schemasBycat = await Promise.all(
    catalogs.map(async (cat) => {
      const data = await ucGet<{ schemas: SchemaInfo[] }>(`/schemas?catalog_name=${cat.name}&max_results=200`)
      return (data?.schemas ?? []).filter((s) => s.name !== 'information_schema')
    }),
  )
  ...
  const details = await Promise.all(
    allTables.map((t) =>
      ucGet<TableDetail>(`/tables/${t.catalog_name}.${t.schema_name}.${t.name}`),
    ),
  )
