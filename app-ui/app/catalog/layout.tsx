...
function Chevron({ open }: { open: boolean }) {
  return (
    <svg
      ...
      className={cn('shrink-0 transition-transform text-muted-foreground', open && 'rotate-90')}
    >
...
export default function CatalogLayout({ children }: { children: React.ReactNode }) {
  ...
  async function handleCatalog(cat: string) {
    ...
    if (!schemas[cat]) {
      const d = await apiFetch<{ schemas: Schema[] }>({ type: 'schemas', catalog: cat })
  ...
  async function handleSchema(cat: string, sch: string) {
    ...
    if (!tables[key]) {
      const d = await apiFetch<{ tables: TableSummary[] }>({ type: 'tables', catalog: cat, schema: sch })
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* ── Tree panel ── */}
      <div className="w-60 shrink-0 border-r flex flex-col overflow-hidden select-none">
          ...
          {catalogs.map((cat) => {
            ...
            return (
              <div key={cat.name}>
                ...
                {catOpen && (schemas[cat.name] ?? []).map((sch) => {
                  const schKey = `${cat.name}.${sch.name}`
                  const schOpen = expanded.has(schKey)
                  const schActive = activeCat === cat.name && activeSch === sch.name && !activeTbl
                  ...
                  return (
                    <div key={schKey}>
                      ...
                      {schOpen && (tables[schKey] ?? []).map((tbl) => {
                        const tblActive = activeCat === cat.name && activeSch === sch.name && activeTbl === tbl.name
