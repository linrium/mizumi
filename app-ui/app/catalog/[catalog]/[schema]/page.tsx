...
async function fetchTables(catalog: string, schema: string): Promise<TableSummary[]> {
  const res = await fetch(`/api/catalog?${new URLSearchParams({ type: 'tables', catalog, schema })}`)
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-4 border-b shrink-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">{tables.length} table{tables.length !== 1 ? 's' : ''}</p>
      </div>
          ...
          <table className="w-full text-xs border-collapse">
            <thead className="sticky top-0 bg-muted/80 backdrop-blur-sm">
              <tr>
                ...
                <th className="px-5 py-2 text-left font-medium text-muted-foreground border-b">Type</th>
              </tr>
            </thead>
            <tbody>
              {tables.map((tbl) => (
                <tr key={tbl.name} className="border-b border-border/60 last:border-0 hover:bg-accent/30 transition-colors">
                  ...
                  <td className="px-5 py-2 text-muted-foreground">{tbl.table_type}</td>
                </tr>
              ))}
              {tables.length === 0 && (
                <tr>
                  <td colSpan={2} className="px-5 py-8 text-center text-muted-foreground">No tables found</td>
                </tr>
              )}
            </tbody>
          </table>
